...
# ╔═╡ 549634a4-aabc-11ec-0166-0f3c22f090fb
begin
	using HypertextLiteral, LinearAlgebra, Plots, PlutoUI, JLD2, JSON, CodecZlib, CategoricalArrays, DataFrames, SimilaritySearch, SimSearchManifoldLearning,TextSearch
	import Downloads: download
end

# ╔═╡ 1d5266bf-680c-4edb-827d-e9c97c5381f3
md"""
# Visualization of a emoji description in Spanish tweets

This example visualizes a medium sized tweet collection with emojis being masked and used as labels. It uses a simple bag of words representation.
"""

# ╔═╡ d1fe2d7d-a2f8-49f2-a72c-84a079236171
begin
	first_time = Ref(true)
...
# ╔═╡ d3d3de55-0bc8-418f-a9f5-0d90bc7628a6


# ╔═╡ fd294f88-5468-4d37-b835-764824aecd8d


# ╔═╡ 36231efd-6a72-459e-865d-bd08a64a306c
md"""
## Processing the collection into a vector dataset
"""

# ╔═╡ 4f61c0b4-0fb9-47c5-b44c-6ee07215e866
begin

function create_dataset(dbfile)
	if isfile(dbfile * ".jld2")
		vectors, labels, tok, model = load(dbfile * ".jld2", "vectors", "labels", "tok", "model")
		return (tok=tok, labels=labels, model=model, vectors=vectors)
	end
	labels = String[]
	corpus = String[]
	open(dbfile) do f
	...
	tok = Tokenizer(TextConfig(group_usr=true, group_url=true, del_diac=true, lc=true, group_num=true, nlist=[], qlist=[4]))
	...
	jldsave(dbfile * ".jld2"; tok, labels, model, vectors)
	(tok=tok, labels=labels, model=model, vectors=vectors)
end
	
function create_or_load_index(db, indexfile)
    if isfile(indexfile)
...
# ╔═╡ cc34b119-4c85-4d40-bf79-f422cfede648
begin
    ...
    index = create_or_load_index(D.vectors, indexfile)
	
end


# ╔═╡ 2381dbee-bd4f-4b7f-a5de-0e1ef93e4316
function load_or_create_umap_embeddings(index, umapfile; force=true, k=50, n_epochs=50, neg_sample_rate=3, layout=KnnGraphLayout())
	if !force && isfile(umapfile)
		...
		U2 = fit(UMAP, index; k, neg_sample_rate, layout, n_epochs)  # spectral layout is too slow for the input-data
		U3 = fit(U2, 3; neg_sample_rate, n_epochs)  # reuses U2
		jldsave(umapfile, e2=U2.embedding, e3=U3.embedding)
		(e2=U2.embedding, e3=U3.embedding)
	end
end

# ╔═╡ 881bcfc1-907c-4110-98fa-69de65544e57
begin
	umap_button
	...
	function plot_umap()
		e2, e3 = load_or_create_umap_embeddings(index, umapfile; force=false, k=50, n_epochs=50, neg_sample_rate=3, layout=KnnGraphLayout())
		...
		#@show levels(D.labels)
		A = Dict()
		for a in levels(D.labels)
			A[a] = RGB(rand(), rand(), rand())
		end
		A = [A[a] for a in D.labels]
		scatter(X, Y, c=A, fmt=:png, size=(600, 600), ma=0.3, a=0.3, ms=1, msw=0, label="")
...
[[deps.InvertedFiles]]
deps = ["Intersections", "LinearAlgebra", "Random", "SimilaritySearch", "SparseArrays"]
git-tree-sha1 = "f71c99829a9cd67782ef1261bc9917c633a0d924"
...
git-tree-sha1 = "2987a4f019912c188461c4860c1c0e5dd272c51d"

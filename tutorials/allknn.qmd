...
5. Solving allknn with the `G` index; it returns a matrix with nearest neighbor identifiers and their corresponding distances, each entry is a `IdDist` structure with two entries `id` and `dist`, the later being the associated distance. Note that `allknn` preserves the self reference.

...
k = 8 # <2>
etime = @elapsed gold_knns = allknn(ExhaustiveSearch(; B.db, B.dist),  GenericContext(), k) # <3>
```
1. Creates a synthetic dataset of dimension $16$ and $10^5$ points.
2. The number of neighbors to be fetched.
3. Creates a gold standard for testing and comparing.
...
atime = @elapsed knns = allknn(G, ctx, k) # <4>
```
...
5. Solving allknn with the `G` index; it returns a matrix with nearest neighbor identifiers and their corresponding distances, each entry is a `IdWeight` structure with two entries `id` and `weight`, the later being the associated distance. Note that `allknn` preserves the self reference.

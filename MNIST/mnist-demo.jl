...
using PlutoUI, SimilaritySearch, JLD2, DataFrames, HypertextLiteral, LinearAlgebra, Plots, SimSearchManifoldLearning, MLDatasets
...
function load_or_create_umap_embeddings(index; force=false)
	if !force && isfile(umapfile)
		...
		U2 = fit(UMAP, index; maxoutdim=2, k, neg_sample_rate, layout, n_epochs)
...
begin
	...
	function plot_umap(index, labels; force=true)
		e2 = load_or_create_umap_embeddings(index; force)
		X = @view e2[1, :]
	    Y = @view e2[2, :]
...
# ╔═╡ e9b928dc-433d-4b1c-8a38-50563ed065d1
L2Distance

...
begin
	...
	using PlutoUI, SimilaritySearch, SimSearchManifoldLearning, JLD2, DataFrames, HypertextLiteral, LinearAlgebra, Plots, MLDatasets
end

# ╔═╡ 5cd87a9e-5506-11eb-2744-6f02144677ff
md"""
# UMAP projection dynamic on MNIST


This example shows an _advanced_ visualization of a series of UMAP projections of the MNIST database.
> LeCun, Y., Bottou, L., Bengio, Y., & Haffner, P. (1998). Gradient-based learning applied to document recognition. Proceedings of the IEEE, 86(11), 2278-2324.

The MNIST database is a set $60,000$ $28\times 28$ gray-scale images of handwritten digits, each object is labeled with one of the ten available labels, i.e., the written digit.

## About the visualization
This example shows a series of 2d projections that form an animation of the projection's dynamic when the number of neighbors is varied.
This procedure provides insight into relations and similarities between classes of objects in a dataset, the MNIST in this case.

We link colors to each class and also plot the label of a small sample of objects. Currently, the typography does not correspond with the actual hand-written digit.

## On the initial layout
For this example, we used a random initial layout instead of the Spectral layout that can produce a much better visualization but uses more computational resources. More importantly, it reduces the insight since it produces relatively stable and well-clusterized components on a few neighbors.

## About the computational cost
Note that even when this is a computational intensive visualization, it performs relatively fast in a multithread system. For instance, I ran this example on a workstation with Intel(R) Xeon(R) Silver 4216 CPU @ 2.10GHz (32 cores with hyperthreading), the initial projections take a few seconds while the last ones took close to 25 seconds, taking a bit more than 4 minutes in the whole animation.
...
function plot_umap(index, labels;
	...
	knns, dists = allknn(index, last(k_list); parallel=true)
	precindex = PrecomputedKnns(knns, dists)
	E = fit(UMAP, precindex; k=first(k_list), neg_sample_rate, layout, n_epochs=100)
	layout = PrecomputedLayout(E.embedding)
	L = rand(1:60000, 100)
	anim = @animate for k in k_list
		@info "=== $k ==="
		E = fit(UMAP, precindex; k, neg_sample_rate, layout, n_epochs)
		layout = PrecomputedLayout(E.embedding)
		X = @view E.embedding[1, :]
	    Y = @view E.embedding[2, :]

		scatter(X, Y, c=labels, fmt=:png, size=(600, 600), ma=0.3, a=0.3, ms=1, msw=0, label="neighbors = $k", yticks=nothing, xticks=nothing, xaxis=false, yaxis=false)
		
		for i in L
			annotate!(X[i], Y[i], text(string(labels[i]), :black, 0.5, 12))
		end
	end
...
Click the button if you want to compute a 2d projection of the dataset. This can take a long time if you don't run the notebook with several threads.
...
# ╔═╡ 570b81e6-25bf-448b-9322-4011848ad574
md"""
The series of projections go from a small number of neighbors with points already clustered but many components along the plane. As the number of neighbors increases, the number of components is reduced and the compactness of components is also increased. 
For more than 100 neighbors, the animation is quite stable, but also maintains its tendency of reducing components.

While this example doesn't go beyond and see actual examples of classes, it is not hard to imagine similar _4_ and _9_ digits, or _7_ and _9_ digits being quite similar.
"""
...
git-tree-sha1 = "4e7f332afbac29d8e4b6e5d1cf49a51dae9ae7f0"
...
[[deps.Intersections]]
deps = ["Random"]
git-tree-sha1 = "4f8b1a0796b487d3db2c3579da7e7147de013faa"

...
This notebook visualizes a 2d and 3d projections made with UMAP (3d as RGB). The input of UMAP is an all-knn graph created with SimilaritySearch.jl.

To use this Pluto script, you must run first some of the `create-index-and-umap.jl` scripts that are in the subfolders.

"""

# ╔═╡ a2a8710b-f7ed-4454-b00d-f0e92c78298a
begin
	embradio = @bind embfile Select([p => dirname(p) for p in glob("*/*umap-embeddings.jld2")])

	lim_slider = @bind lim Slider(2:30, default=15)

	@htl """
	<div>
	Please select some embeddings:
	$(embradio), zoom: $(lim_slider)
	</div>
	<div style="background-color: rgb(230, 230, 230);">
	Note: if empty, please create some UMAP embeddings first.
	</div>	
	"""
end
...
begin
	e2, e3 = load(embfile, "e2", "e3")
	size(e2), size(e3), embfile
end

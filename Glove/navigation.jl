...
begin
	using SimilaritySearch, HypertextLiteral,JLD2, PlutoUI, Colors 
end
...
begin
	n = length(index)
	wordinput = @bind example_id Slider(1:n, default=rand(1:n))
	md"""
...
begin
	@time R = search(index, index[example_id], SimilaritySearch.getknnresult(10))
	L = []
	for (i, (id, d)) in enumerate(R.res)
		push!(L, @htl "<tr><td>$i</td><td>$(vocab[id])</td><td>$id</td><td>$(round(d, digits=3)) </td></tr>")
	end
	@htl """

<h2>Query</h2>
query: $(vocab[example_id]), id=$(example_id), n=$(length(index))
<table>
<tr><td>knn</td><td>word</td><td>id</td><td>dist</td></tr>
$L
</table>

...
begin
	using SimilaritySearch, SimSearchManifoldLearning, HypertextLiteral,JLD2, PlutoUI, Colors, LinearAlgebra , Strs, DataFrames, Plots
...
# ╔═╡ eaf83cde-4973-4f23-9c35-01a98f24bac4


# ╔═╡ 223a514e-ee14-4660-a629-dd45b53063ca


# ╔═╡ 1ce583f6-54fb-11eb-10ad-b5dc9328ca3b
begin
	database = "enwiktionary.jld2"
...
begin
	@htl """
	<button onclick='var elem = document.querySelector("bond >input"); elem.value = "_random"; elem.dispatchEvent(new CustomEvent("input"));'>random word</button>
...
begin
query_input = @bind query TextField(; default="julius")
...
function displayresults(q, k=10, res=knnres)
	reuse!(res)
	...
	for (i, (id, d)) in enumerate(res)
		word = index[id]
		push!(L, @htl """
		<tr><td>$i</td><td>$(word)</td><td>$id</td><td>$(round(d, digits=3)) </td><td><a href="https://en.wiktionary.org/wiki/$(word)" target="_blank">meaning</a></td><td><button onclick='var elem = document.querySelector("bond >input"); elem.value = "$word"; elem.dispatchEvent(new CustomEvent("input")); '>similar</button></td></tr>
		""")
	end
...
<h2>Query</h2>
query: $(query), n=$(length(index))
<table>
<tr><td>knn</td><td>word</td><td>id</td><td>dist</td><td>wiktionary</td><td></td></tr>
$L
</table>
...
begin
	n = length(index)
	q = query == "_random" ? index[rand(1:n)] : UTF32Str(query)
	displayresults(q)
end

# ╔═╡ 97499f52-1cbb-413e-8381-09f2faeff881
function load_or_create_umap_embeddings(index; force=true, k=50, n_epochs=50, neg_sample_rate=3, layout=KnnGraphLayout())
	if isfile(umapfile)
		...
		U2 = fit(UMAP, index; k, neg_sample_rate, layout, n_epochs)  # spectral layout is too slow for the input-data's size
...
# ╔═╡ 5ca2c677-cb08-4243-84bd-8987e7f9b029
begin
	first_time = Ref(true)
...
# ╔═╡ 52c28284-f1d0-4df9-8709-fd7ca08b2b43
begin
	umap_button
	...
	function plot_umap()
		e2, e3 = load_or_create_umap_embeddings(index; force=true, k=15, n_epochs=50, neg_sample_rate=3, layout=KnnGraphLayout())

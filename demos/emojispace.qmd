...
> **TextSearch 1.1 note:** `TextConfig` now accepts `NormalizationConfig` and `TokenizationConfig`
> sub-structs for fine-grained control. Flat keyword arguments (e.g. `group_usr`, `nlist`) are kept
> for backward compatibility and are used below.

```{julia}
#| output: false
using SimilaritySearch, SimilaritySearch.Dist, SimSearchManifoldLearning, TextSearch, CodecZlib, JSON, DataFrames, Plots, StatsBase
...
# TextSearch 1.1: flat kwargs are still supported as a convenience interface.
# Equivalent structured form:
#   textconfig = TextConfig(
#       norm=NormalizationConfig(del_diac=true, lc=true),
#       tok=TokenizationConfig(group_usr=true, group_url=true, group_num=true, nlist=[1], qlist=[3]))
textconfig = TextConfig(
    group_usr=true,
    group_url=true,
    del_diac=true,
...
e2, e3 = let min_dist=0.5f0, # <1>
    ...
    index = ExhaustiveSearch(dist, rand(vectors, indexsize)) # <2>
